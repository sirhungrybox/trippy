...
import { Map, CalendarDays, Wallet, CheckSquare, LogOut } from 'lucide-react'
...
export function TopBar({ trip, tab, onTab, onReset }: { trip: Trip; tab: Tab; onTab: (t: Tab) => void; onReset: () => void }) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b" style={{ borderColor: 'var(--color-surface-overlay)' }}>
      <div className="max-w-5xl mx-auto px-4 flex items-center justify-between h-13">
        <button onClick={() => onTab('overview')} className="flex items-center gap-2.5 shrink-0 min-h-[44px]">
          <span className="text-lg">{trip.coverEmoji}</span>
          <span className="text-sm font-semibold truncate max-w-[140px] sm:max-w-[200px]" style={{ color: 'var(--color-text-primary)' }}>{trip.name}</span>
        </button>
        <div className="hidden sm:flex items-center gap-0.5">
          ...
          <button onClick={onReset} className="ml-3 p-2.5 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg"
        ...
        <button onClick={onReset} className="sm:hidden p-2.5 active:text-red-400 min-h-[44px] min-w-[44px] flex items-center justify-center"
          style={{ color: 'var(--color-text-faint)' }}>
          <LogOut size={16} />
        </button>
      </div>
